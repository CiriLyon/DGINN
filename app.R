...
ui = fluidPage(
  ...
  #_________________________________fluidRow de l'espace de téléversement du document à traiter____________________________________________________ 
  fluidRow(
    sidebarLayout(
  ...
  fluidRow(
    mainPanel(
      column(
        ...
        align = "right" ,
        offset = 3 ,
        plotOutput("sitePos" ,height = "2800px" )
      )#column
    )#mainPanel)
  ),#fluidRow(
  #________________________fin fluidRow sitePos_____________________________________________________________
  
  #________________________fluidRow vide, jsute pour ne pas arrêter la page pile à la fin du dernier graphique_____________________________________________________________
...
server = function(input , output ,session){
  ...
  observeEvent(input$boutonUpload,{
    ...
    legend("topright", legend  = TAB4_col$Nom , fill = TAB4_col$coul, cex  = 1 , horiz = T , title = "Used Methods",
           bg = NA,
           bty = "n",
           title.col = "indianred" ,
    ...
    for(t in 1:length(lstGene)){
      ...
      for(CC in 1:ncol(X5)){
        ...
        if(CC == 1){
          ...
          mtext(names(lstGene)[t],side=2,las=1,line=0, cex  = 1 )
        }
    ...
    output$sitePos <- renderPlot({
      ...
      #-- La légende
      plot(0,0, axes  = F , xlim =  c(0 , maxPos), ylim = c(0,maxDet+1 ), bty="n", ylab ="",type ="n") #main = names(lstGene)[1],
      legend("topleft", legend  = TAB4_col$Nom , fill = TAB4_col$coul, cex  = 1.5 , horiz = F , title = "Used Methods",
             bg = NA,
             bty = "n",
             title.col = "indianred" ,
             text.col = TAB4_col$coul ,
